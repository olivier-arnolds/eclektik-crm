...
export default async function handler(req, res) {
  ...
  const { data: due, error: dErr } = await supabase
    ...
    .select('id,email,email_domain,status,next_action_at,priority_tier,outreach_prio,unsubscribe_token,msg1_subject,msg1_body,msg2_subject,msg2_body')
  ...
  const fromHeader = senderName && senderName !== camp.sender_mailbox
    ...
    : camp.sender_mailbox;
  const stats = { sent: 0, claimed_elsewhere: 0, failed: 0 };
  const failures = [];
  ...
  for (const item of batch) {
    ...
    const { data: claim, error: claimErr } = await supabase
      ...
      .insert({
        ...
        to_address: c.email, from_address: camp.sender_mailbox, subject: item.subject,
      })
      .select('id').single();
    if (claimErr) {
    ...
    if (!send.ok) {
      ...
      stats.failed++;
      failures.push({
        contact_id: c.id, email: c.email, step: item.step, status: send.status,
        error: (send.data?.message || JSON.stringify(send.data || {})).slice(0, 200),
      });
      continue;
    }
