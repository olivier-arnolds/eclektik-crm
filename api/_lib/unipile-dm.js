...
// Geeft: { ok:true, messageId, chatId, providerId } | { ok:false, status?, error, details? }
//
// providerId mag meegegeven worden als hij al bekend is. Dat scheelt stap 1, en
// dat is geen micro-optimalisatie: elke profielopvraag telt bij LinkedIn als een
// echte profielweergave op het account. Bij outreach naar 150 mensen is dat het
// verschil tussen 150 en 300 acties op Marco's account.
export async function sendLinkedInDM({ accountId, linkedinUrl, text, providerId: knownProviderId = null }) {
  if (!DSN || !TOKEN) return { ok: false, error: 'Unipile not configured (DSN/TOKEN ontbreekt)' };
  if (!accountId || (!linkedinUrl && !knownProviderId)) return { ok: false, error: 'accountId en linkedinUrl of providerId vereist' };
  ...
  // 1) provider_id ophalen (overgeslagen als hij al bekend is)
  ...
  if (!providerId) {
    ...
    providerId = pdata.provider_id || pdata.id || pdata.member_id;
  ...
  const chatId = cdata.chat_id || cdata.id || null;
  const messageId = cdata.message_id || chatId || null;
  return { ok: true, messageId, chatId, providerId, data: cdata };
}
