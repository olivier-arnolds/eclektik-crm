...
//
// Apart van het endpoint omdat ER TWEE AANROEPERS ZIJN: de knop 'verstuur batch'
// (api/outreach-send.js, met een ingelogde gebruiker) en de cron die de dagcap
// over de dag uitsmeert (api/outreach-drip.js). Hier zit de claim-dan-verstuur-
// volgorde, en die mag maar op EEN plek staan: twee kopieen daarvan zouden vroeg
// of laat uit elkaar lopen en dat kost een dubbel verstuurd bericht.
...
export async function runOutreachBatch({ campaign_id, limit, onlyStep = null, dry_run = false } = {}) {
  ...
  if (cErr || !camp) return fout(404, 'campagne niet gevonden');
  ...
  if (!dry_run) {
    if (isLinkedIn && !(process.env.UNIPILE_API_KEY && (process.env.UNIPILE_BASE_URL || process.env.UNIPILE_DSN))) {
      return fout(500, 'Unipile is niet geconfigureerd (UNIPILE_API_KEY / UNIPILE_BASE_URL)');
    }
    if (!isLinkedIn && !process.env.RESEND_API_KEY) {
      return fout(500, 'RESEND_API_KEY not configured');
    }
  }

  // Killswitch (handover §6): alleen een actieve campagne verstuurt.
  if (camp.status !== 'active' && !dry_run) {
    return fout(400, `campagne staat op '${camp.status}'. Zet hem op 'active' om te versturen.`);
  }
  ...
  const { data: due, error: dErr } = await supabase
    ...
    .limit(1000);
  if (dErr) return fout(500, 'kandidaten ophalen: ' + dErr.message);
  ...
  if (dry_run) return { status: 200, body: { ok: true, dry_run: true, plan } };
  ...
  return { status: 200, body: { ok: true, plan, stats, failures: failures.slice(0, 25) } };
}
