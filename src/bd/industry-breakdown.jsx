...
// array as a prop (lane-reporting.jsx fetches companies; `industry` is added to
// that select by this same setup).
...
export default function IndustryBreakdown({ companies = [] }) {
  const { sectors, totalClients, totalProspects, maxVal } = useMemo(() => {
    ...
    for (const c of companies) {
      ...
      acc[s] = acc[s] || { sector: s, clients: 0, prospects: 0 };
      if (c.type === 'Customer') { acc[s].clients += 1; tc += 1; } else { acc[s].prospects += 1; tp += 1; }
    }
    ...
    <div style={card}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', gap: 12, flexWrap: 'wrap' }}>
        <div>
          ...
          <div style={{ fontSize: 12, ...muted, marginTop: 2 }}>Accounts grouped into broad sectors. Client = type Customer, prospect = type Prospect.</div>
        </div>
      ...
      <div style={{ marginTop: 12, display: 'flex', flexDirection: 'column', gap: 9 }}>
        {sectors.map((s) => (
          <div key={s.sector} style={{ display: 'grid', gridTemplateColumns: '210px 1fr', alignItems: 'center', gap: 12 }}>
            <div style={{ fontSize: 12, color: 'var(--text-2)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }} title={s.sector}>{s.sector}</div>
